...
class OrderController extends Controller
{
        ...
        return DataTables::of($orders)
            ...
            ->addColumn('actions', function ($q) use ($request) {
                ...
                $answers = DB::table('user_answer')
                            ->join('orders', 'orders.user_id', '=', 'user_answer.user_id')
                            ->join('questions', 'user_answer.question_id', '=', 'questions.id')
                            ->select('user_answer.answer', 'questions.id as q_id', 'questions.question')
                            ->where('orders.id', $q->id)
                            ->where('questions.questiontype','7')
                            ->get();
                if($answers->count() > 0){
                    foreach($answers as $one) {
                        $data = json_decode($one->answer, true);
                        $flag = 0;
                        foreach($data as $image) {
                            ...
                            $view .= view('backend.datatable.action-download')
                                ->with(['route' => route('admin.orders.download', ['course_id' => $q->items[0]->item_id,'user_id'=>$q->user->id, 'question_id'=>$one->q_id]), 'question'=>$one->question])->render();
                        }
                    }
                }
                // if($flag) {
    ...
    public function download($id,$user_id,$question_id)
    {
        ...
        //     ->get();
        $files = DB::table('user_answer')
            ->select('user_answer.answer')
        ...
        if($files->count() > 0){
            ...
            if ($zip->open(public_path('storage'.DIRECTORY_SEPARATOR.'temp'.DIRECTORY_SEPARATOR.$fileName), \ZipArchive::CREATE)== TRUE)
            {
                ...
                foreach ($files as $key => $value){
                        $fs = json_decode($value->answer, true);
                        if(is_array($fs)) {
                            foreach($fs as $f){
            ...
            header("Content-type: ".mime_content_type(public_path('storage'.DIRECTORY_SEPARATOR.'temp'.DIRECTORY_SEPARATOR.$fileName))); 
