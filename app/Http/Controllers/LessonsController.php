...
class LessonsController extends Controller
{
    ...
    public function show($course_id, $lesson_slug)
    {
        ...
        $default = DB::table('defsetting')->select('*')->first();
        // $lesson->questions = $lesson->questions->sortBy('page_number');
        // dd($answers);
        return view($this->path . '.courses.lesson', compact('lesson', 'default', 'previous_lesson', 'next_lesson', 'test_result',
    ...
    public function user_upload_files(Request $request)
    {
        ...
        $image_types_str = json_decode($question->content)[0]->file_type;
        $allow_size = json_decode($question->content)[0]->file_dimention * 1000000;
        $image_types = explode(',', $image_types_str);
                    ...
                    {   
                        ...
                        if(in_array($extension, $image_types)){
                            if($file_size <= $allow_size) {
                                ...
                                $output = array(
                                    'result'  => 'Dimensione non valida. (massimo:'.json_decode($question->content)[0]->file_dimention.'MB)',
                            ...
                            $output = array(
                                'result'  => 'Estensione non valida. ('.$image_types_str.')',
                                'file_names' =>$file_names,
                                'q_id' => $request->q_id
                            );
                            return response()->json($output);
