var QuestionCreate = function() {
    ...
    **/
    ...
    $("#add_col").on('click', function(e) {
        if($('#add-matrix th').length == 0){
            ...
            if($(".selecttype").val() == "radio"){
                ...
                scoreinput = '<input type="text" data-q_id="q_id'+col_add+'" data-value="" class="form-control d-inline radioscore" value=""  onchange="radioScore(this)">';
                radiocol = 'col-2';
            }
    ...
    $('#save_data').on('click', function(e) {
        ...
        var quiz_img = $('#quiz_img').val();
        var img_width = $('#image_width').val();
        var img_height = $('#image_height').val();
        if(quiz_img && (!img_width || !img_height)) {
            swal("error","Type Image Size","error");
            return;
        }
        switch(type_id) {
            //Single Input
            case 0:
                ...
                $label = $("#single_input_label").val();
                $alignment = $("#single_input_alignment").val();
                $type = $("#single_input_type").val();
                $size = $("#single_input_size").val();
                $mtop = $("#single_input_mtop").val();
                temp_arr.push({
                    'label':$label,
                    'alignment':$alignment,
                    'type':$type,
            ...
            case 1:
                ...
                // $val = $("#p_use").val();
                $val = '';
                if(window.p_use/2) $val = 'checked';
                temp_arr.push({
                    'col' : $("#display_checkbox").val(),
                    'checkbox_color' : $("#checkbox_color").val(),
                    'p_use' : $val
                });
                temp_arr.push({
            ...
            case 4:
                ...
                var cnt = $('#add-matrix tr th').length - 1;
                // var width = 100/cnt + '%';
                // var width = 100+'%';
                $data_id = $('#add-matrix').attr('data-id');
                $('#add-matrix #header_row_col1').css("background-color", $('#default_background_color').val())
                $('#add-matrix #header_row_col1 th label').css({"color": $('#default_font_color').val()+"!important", "font-size": $('#default_font_size').val()+"px", "font-style": $('#default_font_style').val(), "font-family": $('#default_font_family').val()});
                $('#add-matrix td label').css({"color": "#000000 !important", "font-style": $('#default_font_style').val(), "font-family": $('#default_font_family').val()});
                $('#add-matrix .custom-border').css("text-align", "center");
                // $('#add-matrix tr th').css("width", width);
                // $('#matrixbox_style').text($style);
            ...
            case 6:
                ...
                temp_arr.push({
                    'dropdown_size' : $("#dropdown_size").val(),
                });
                temp_arr.push({
                    'dropdown_alignment' : $("#dropdown_alignment").val(),
                });
            ...
            case 7:
                ...
                var temp_arr = [];
                $value = $("#num_files").val();
                if(!$value || !$('#file_dimention').val() || !$('#file_type').val()) {
                    swal("Error!", "Fill in the question type correctly!", "error");
                    return;
                }
                temp_arr.push({
                    'number':$value,
                    'file_dimention':$('#file_dimention').val(),
            ...
            case 9:
                ...
                temp_content['range_color'] = $("#range_color").val();
        ...
        var answer_alignment = $("#answer_alignment").val();
        var image_alignment = $("#image_alignment").val();
        ...
        if($("#question_id").val()){
         ...
         //fin ajax 
            $.ajax({
                   ...
                   'answer_alignment' : $("#answer_alignment").val(),
                   ...
                   'titlecolor': $("#title_color").val() ?? null,
                   'titlelocation': $("#titlelocation").val() ?? null,
                   'help_info_location': $("#help_info_location").val() ?? null,
                   ...
                   'default_font_style': $("#default_font_style").val() ?? null,
                   'default_font_family': $("#default_font_family").val() ?? null,
                   'default_font_size': $("#default_font_size").val() ?? null,
                   'default_font_color': $("#default_font_color").val() ?? null,
                   'default_background_color': $("#default_background_color").val() ?? null,
                   
                   'size': $("#fontsize").val() ?? null,
